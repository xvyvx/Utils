...
private:
        ...
        , typename BaseType::ptr_t &timer, FuncType &func, Args&...args)
    {
        ...
        {
            QueueThreadPoolWorkItem(std::forward<FuncType>(func), std::forward<Args>(args)...);
        }
    }
...
public:
     ...
     */
    template<typename DurationType, typename FuncType, typename... Args> bool QueueThreadPoolWorkItemAfter(DurationType duration, FuncType &&func
        , Args&&... args)
    {
        ...
        typename BaseType::ptr_t::element_type *rawPtr = timer.get();
        boost::system::error_code err;
        ...
        rawPtr->async_wait(std::bind(&TimerCacheBase::OnThreadPoolWorkItemDelayTimer<FuncType, Args...>, std::placeholders::_1, std::move(timer)
