...
BOOST_AUTO_TEST_CASE(GeneralTest)
{
    ...
    linearBuf.assign(8, 0);
    BinaryHelper::SetBinData(linearBuf.data(), 0, 1.8f, true);
    tempFloat = BinaryHelper::ReadBinData(linearBuf.data(), 0, true);
    BOOST_TEST(tempFloat == 1.8f);
    BinaryHelper::SetBinData(linearBuf.data(), 0, 1.8f, false);
    tempFloat = BinaryHelper::ReadBinData(linearBuf.data(), 0, false);
    BOOST_TEST(tempFloat == 1.8f);
    ...
    for (us8 i = 1; i <= 4; ++i)
    {
        ...
        BinaryHelper::SetBinData(unalignPtr, 0, 0x10, i, false);
        temp = BinaryHelper::ReadBinData(unalignPtr, 0, i, false);
        BOOST_TEST(temp == 0x10);

        BinaryHelper::SetBinDataSigned(unalignPtr, 0, -1, i, true);
        s32 tempSigned = BinaryHelper::ReadBinDataSigned(unalignPtr, 0, i, true);
        BOOST_TEST(tempSigned == -1);

        BinaryHelper::SetBinData(unalignPtr, 0, 1.8f, true);
        tempFloat = BinaryHelper::ReadBinData(unalignPtr, 0, true);
        BOOST_TEST(tempFloat == 1.8f);
        BinaryHelper::SetBinData(unalignPtr, 0, 1.8f, false);
        tempFloat = BinaryHelper::ReadBinData(unalignPtr, 0, false);
        BOOST_TEST(tempFloat == 1.8f);
    }
}
